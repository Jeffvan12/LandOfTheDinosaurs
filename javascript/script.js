...
document.addEventListener("DOMContentLoaded", () => {
        ...
        if (document.cookie !== '') {
            ...
            for (let dino in dinoseen) {
                if (dinoseen[dino]) {
                    loadTemplate(dino);
                    num++;
                    // document.getElementById(collectiondinotemplatemappings[dino]).addEventListener("click", () => {
            ...
            document.getElementById("unlockedbutton").innerHTML = num + "/2 Dinosaurs unlocked";
        } else {
            document.getElementById("unlockedbutton").innerHTML = "0/2 Dinosaurs unlocked";
        }
    ...
    document.getElementById("logodiv").addEventListener("click", () => location.href = 'worldmap.html');

});

//Making the cookie and saving it 
function makeCookie() {
    let tosave = {
    ...
    document.cookie = JSON.stringify(tosave);
}

//Loading the cookie file 
function loadCookie() {
    let toload = JSON.parse(document.cookie);
